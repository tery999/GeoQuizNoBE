...
import * as styles from "./EuropeOutlines.module.css"
...
export default function EuropeOutlinesPlus () {
  ...
  const [reload, setReload] = useState(false);
    const [ countryArrShuffled ,choices, 
        ...
        loaded, correctAsnwers, totalAnswers, currentTurn] = useCustomOutlines(getEuropeDataPlus, reload);
    ...
    const choiceClickFunction = (name) => {
      ...
      if (loaded && currentTurn.current > totalAnswers.current ) {
        changeScoreFunc({EuroOutlinesPlus: correctAsnwers.current});
      }
    }
