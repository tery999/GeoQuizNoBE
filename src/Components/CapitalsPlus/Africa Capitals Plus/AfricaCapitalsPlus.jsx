...
export default function AfricaCapitalsPlus () {
  ...
  const [reload, setReload] = useState(false);
    const [ countryArrShuffled, choices, setChoices, currentCapital, setCurrentCapital, loaded, correctAsnwers, totalAnswers, currentTurn] = useCustomCapitals(getAfricaDataPlus, reload);
    ...
    const choiceClickFunction = (name) => {
      ...
      if (loaded && currentTurn.current > totalAnswers.current ) {
        debugger;
        changeScoreFunc({AfricaCapitalsPlus: correctAsnwers.current});
      }
    }
